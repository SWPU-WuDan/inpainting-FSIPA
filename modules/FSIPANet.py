...
from modules.FSILayer import FSILayer
import modules.ContextualAttention as Attention
import torch
...
class FSIPANet(BaseNetwork):
    def __init__(self, layer_size=8, input_channels=3, att = False):
        super().__init__()
        self.layer_size = layer_size
        self.enc_1 = FSILayer(3, 64, kernel_size = 7)
        self.enc_2 = FSILayer(64, 128, kernel_size = 5)
        ...
        if att:
            self.att = Attention.PixelContextualAttention(256)
        else:
            self.att = lambda x:x
        self.dec_3 = PConvLayer(256 + 128, 128, activ='leaky', deconv = True)
        self.dec_2 = FSILayer(128 + 64, 64, stride = 1, activation='leaky', deconv = True)
        self.dec_1 = FSILayer(64 + input_channels, 64, stride = 1, activation = None, batch_norm = False)
