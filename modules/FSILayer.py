...
class FSGenerator(nn.Module):
    def __init__(self, in_channels_feature, kernel_s = 3, add_last_edge = True):
        ...
        self.p_conv = PartialConv2d(in_channels_feature + 4, 64, kernel_size = kernel_s, stride = 1, padding = kernel_s // 2, multi_channel = True, bias = False)
        
        self.edge_resolver = Bottleneck(64, 16)
        self.out_layer = nn.Conv2d(64, 4, 1, bias = False)
...
class FSILayer(nn.Module):
    def __init__(self, in_channel, out_channel, stride = 2, kernel_size = 3, batch_norm = True, activation = "ReLU", deconv = False):
        super(FSILayer, self).__init__()
        self.edge_generator = FSGenerator(in_channel, kernel_s = kernel_size)
        self.feat_rec = PartialConv(in_channel+4, out_channel, stride = stride, kernel_size = kernel_size, padding = kernel_size//2, multi_channel = True)
        if deconv:
    ...
    def forward(self, feat_in, mask_in, edge_in):
        ...
        mask_in1=torch.cat([mask_in[:,:3,:,:],mask_in[:,:1,:,:]], dim = 1)
        edge_updated, mask_updated = self.edge_generator(torch.cat([feat_in, edge_in], dim = 1), torch.cat([mask_in, mask_in1[:,:4,:,:]], dim = 1))
        edge_reconstructed = edge_in * mask_in1[:,:4,:,:] + edge_updated * (mask_updated[:,:4,:,:] - mask_in1[:,:4,:,:])
        feat_out, feat_mask = self.feat_rec(torch.cat([feat_in, edge_reconstructed], dim = 1), torch.cat([mask_in, mask_updated[:,:4,:,:]], dim = 1))
